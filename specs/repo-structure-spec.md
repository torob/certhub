...
- Server deployment manifests should mount or generate the YAML server config file and provide its path through `certhub-server run --config <path>` or `CERTHUB_SERVER_CONFIG`. Manifests that should apply migrations during startup must pass `--migrate` explicitly.
